...
{
    public class LifeSaving : Controller
    {
        ProjectDbContext _context;
        public LifeSaving(ProjectDbContext context)
        {
            _context = context;
        }
        ...
        {
            //chặn bằng ss
            if (HttpContext.Session.GetInt32("role") == null || HttpContext.Session.GetInt32("role") != 3)
            {
        ...
        public async Task<IActionResult> lSaving() 
        {
        ...
        public async Task<IActionResult> donor()
        {
            ...
            //LinQ
            var donor = from ls in _context.User
                                 join lsRole in _context.UserRoles! on ls.Id equals lsRole.UserId
                                 where lsRole.RoleId == 1
                                 select new
                                 {
                                     Id = ls.Id,
                                     Name = ls.Name,
                                     City = ls.City,
                                     BloodGroup = ls.BloodGroup,
                                 };
            ViewBag.donor = donor;
        ...
        [HttpPost]
        public async Task<IActionResult> Create(User data, UserRoles role)
        {
            //chặn bằng ss
            if (HttpContext.Session.GetInt32("role") == null || HttpContext.Session.GetInt32("role") != 3)
            {
